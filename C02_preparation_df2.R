...
#quanti hanno gmail, libero, alice, hotmail...
...
#numero di clienti che hanno aggiunto il numero di telefono
df_2_cli_account_w_phone <- df_2_cli_account_clean %>%
  group_by(W_PHONE) %>%
  summarise(TOT_PHONE = n())%>%
  mutate(PERCENT = TOT_PHONE/sum(TOT_PHONE)) %>%
  arrange(desc(PERCENT)) %>%
  as.data.frame()

df_2_cli_account_w_phone

#raggruppamento dei clienti in base al tipo di accaunt
df_2_cli_account_typ_cli_account<- df_2_cli_account_clean %>%
  group_by(TYP_CLI_ACCOUNT) %>%
  summarise(TOT_TYP_CLI_ACCOUNT = n())%>%
  mutate(PERCENT = TOT_TYP_CLI_ACCOUNT/sum(TOT_TYP_CLI_ACCOUNT)) %>%
  arrange(desc(PERCENT)) %>%
  as.data.frame()

df_2_cli_account_typ_cli_account

#raggruppamento dei clienti in base al lavoro che fanno
df_2_cli_account_typ_job<- df_2_cli_account_clean %>%
    group_by(TYP_JOB) %>%
  summarise(TOT_TYP_JOB = n())%>%
  mutate(PERCENT = TOT_TYP_JOB/sum(TOT_TYP_JOB)) %>%
  arrange(desc(PERCENT)) %>%
  as.data.frame()

df_2_cli_account_typ_job
#-------------------------------------------------------------------------
...
df_2_dist_emailprovider %>%
  ...
  as.data.frame() %>%
  head(20)
#percent covered è la percentuale cumulata!
...
#rappresentazione grafica dei clienti suddivisi in base al fatto che abbiano o meno aggiunto il numero di telefono
plot_df_2_cli_account_w_phone <- (
  ggplot(data=df_2_cli_account_w_phone
...
#rappresentazione grafica dei clienti suddivisi a seconda del tipo di account
plot_df_2_cli_account_typ_cli_account <- (
  ggplot(data=df_2_cli_account_typ_cli_account
         , aes(x=TYP_CLI_ACCOUNT, y=TOT_TYP_CLI_ACCOUNT)) +
    geom_bar(stat="identity"
             , fill="steelblue") +
    theme_minimal()
)
...
#rappresentazione grafica dei clienti suddivisi in base al lavoro che fanno
plot_df_2_cli_account_typ_job <- (
  ggplot(data=df_2_cli_account_typ_job%>%
           filter(TYP_JOB!='(missing)')
         #ho tolto (missing) perchè sono stratanti rispetto agli altri e grafico esce 0 per tutti gli altri!!
         , aes(x=TYP_JOB, y=TOT_TYP_JOB)) +
    geom_bar(stat="identity"
             , fill="steelblue") +
    theme_minimal()+
